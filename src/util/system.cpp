...
const char * const BITCOIN_CONF_FILENAME = "scancoins.conf";
...
#else
    const char* pszModule = "scancoins";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "scancoins";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/scancoins";
#else
    // Unix
    return pathRet / ".scancoins";
#endif
#endif
}
